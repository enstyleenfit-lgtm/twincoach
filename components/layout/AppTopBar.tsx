...
function getDemoRoleFromCookie(): string | null {
  if (typeof document === "undefined") return null;
  const match = document.cookie
    .split(";")
    .map((c) => c.trim())
    .find((c) => c.startsWith(`${DEMO_ROLE_COOKIE_NAME}=`));
  return match ? match.slice(DEMO_ROLE_COOKIE_NAME.length + 1) : null;
}
...
export function AppTopBar() {
  ...
  useEffect(() => {
    setDemoRole(getDemoRoleFromCookie());
  }, []);
  ...
  const isStoreRole = demoRole === "store";
    ...
    <div className="sticky top-0 z-30 w-full border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex w-full max-w-full items-center justify-between gap-3 px-4 py-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {isStoreRole ? (
            <div className="flex min-h-[44px] min-w-[9rem] sm:min-w-[10rem] items-center rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-900 shadow-sm">
