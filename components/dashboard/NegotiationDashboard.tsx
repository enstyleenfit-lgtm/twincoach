...
export function NegotiationDashboard({
  ...
  return (
    <div className="space-y-20 pb-8">
      {/* 1. Hero + KPI */}
      <section className="relative overflow-hidden rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-900/90 to-zinc-950 px-6 py-10 sm:px-10 sm:py-12">
        ...
        <div className="relative grid gap-10 lg:grid-cols-[1fr_minmax(0,1.1fr)] lg:items-end">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:gap-4">
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/80 px-4 py-5">
              <div className="text-xs font-medium text-zinc-500">会員数</div>
              <div className="mt-2 text-3xl font-bold tabular-nums text-white sm:text-4xl">
                {memberCount}
              </div>
              <div className="mt-1 text-xs text-zinc-600">名</div>
            </div>
            <div className="rounded-xl border border-emerald-500/25 bg-emerald-950/20 px-4 py-5">
              <div className="text-xs font-medium text-emerald-400/90">推定継続率</div>
              <div className="mt-2 text-3xl font-bold tabular-nums text-emerald-400 sm:text-4xl">
                {estimatedRetentionRate}
                <span className="text-lg font-semibold">%</span>
              </div>
            </div>
            <div className="rounded-xl border border-red-500/30 bg-red-950/20 px-4 py-5">
              <div className="text-xs font-medium text-red-300/90">高リスク会員</div>
              <div className="mt-2 text-3xl font-bold tabular-nums text-red-400 sm:text-4xl">
                {highRiskCount}
              </div>
              <div className="mt-1 text-xs text-red-400/50">名</div>
            </div>
            <div className="rounded-xl border border-red-500/35 bg-red-950/25 px-4 py-5">
              ...
              <div className="mt-2 text-2xl font-bold tabular-nums leading-tight text-red-400 sm:text-3xl">
                {formatYen(nextMonthLoss)}
              </div>
              <div className="mt-1 text-[11px] text-red-400/50">30日期待損失の合計</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section aria-label="最重要KPI">
        <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">最重要KPI</h2>
          <p className="text-sm text-zinc-500">経営判断用の主要指標</p>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 px-6 py-8">
            <p className="text-sm font-medium text-zinc-500">会員数</p>
            <p className="mt-3 text-5xl font-bold tabular-nums tracking-tight text-white">
              {memberCount}
            </p>
            <p className="mt-2 text-xs text-zinc-600">登録会員の総数</p>
          </div>
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-950/15 px-6 py-8">
            <p className="text-sm font-medium text-emerald-400/90">推定継続率</p>
            <p className="mt-3 text-5xl font-bold tabular-nums tracking-tight text-emerald-400">
              {estimatedRetentionRate}
          ...
          <div className="rounded-2xl border border-red-500/25 bg-red-950/20 px-6 py-8">
            <p className="text-sm font-medium text-red-300">高リスク会員数</p>
            <p className="mt-3 text-5xl font-bold tabular-nums tracking-tight text-red-400">
              {highRiskCount}
            </p>
            <p className="mt-2 text-xs text-red-400/45">即時フォローが必要な層</p>
          </div>
          <div className="rounded-2xl border border-red-500/30 bg-red-950/25 px-6 py-8">
            ...
            <p className="mt-3 text-4xl font-bold tabular-nums leading-none text-red-400 sm:text-5xl">
              {formatYen(nextMonthLoss)}
            </p>
            <p className="mt-2 text-xs text-red-400/45">30日期待損失・合算</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-red-500/20 bg-gradient-to-b from-red-950/25 to-zinc-950 p-6 sm:p-10">
        <div className="mb-8 max-w-2xl">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">今日の優先対応</h2>
          <p className="mt-2 text-sm text-zinc-400">
            現場が今日使える優先5名。退会確率・理由タグ・次回提案の要点を一目で。
          </p>
        </div>
        {priorityToday.length === 0 ? (
          <p className="text-zinc-500">対象会員はいません</p>
        ) : (
          <ul className="space-y-4">
            {priorityToday.map((row, i) => (
              <li
                key={row.id}
                className="flex flex-col gap-4 rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-5 sm:flex-row sm:items-stretch sm:justify-between"
              >
                <div className="flex min-w-0 flex-1 gap-4">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-red-500/15 text-lg font-bold text-red-400">
                    {i + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <span className="text-xs text-zinc-500">30日退会確率</span>
                      <span
                        className={`text-xl font-bold tabular-nums ${
                          row.probability30Days >= 60 ? "text-red-400" : "text-zinc-200"
                        }`}
                      >
                ...
                <div className="flex flex-1 flex-col justify-between border-t border-zinc-800 pt-4 sm:border-l sm:border-t-0 sm:pl-6 sm:pt-0">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">
                      次回提案AI
                    </p>
                    <p className="mt-1 font-medium text-white">{row.nextTitle}</p>
                    {row.nextDescShort ? (
                      <p className="mt-1 line-clamp-2 text-sm text-zinc-400">{row.nextDescShort}</p>
                    ) : null}
                  </div>
                  <Link
                    href={`/members/${row.id}`}
                    className="mt-4 inline-flex w-fit items-center text-sm font-medium text-blue-400 hover:text-blue-300"
                  >
                    詳細を見る →
                  </Link>
                </div>
      ...
      <section>
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">退会予測ランキング</h2>
          <p className="mt-2 text-sm text-zinc-500">30日・60日確率と想定損失が高い順（上位5名）</p>
        </div>
        {churnRanking.length === 0 ? (
          ...
          <div className="grid gap-4 lg:grid-cols-1">
            {churnRanking.map(({ member, prediction, expectedLoss30Days }, i) => (
              <div
                key={member.id}
                className="flex flex-col gap-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 sm:flex-row sm:items-center sm:justify-between"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">{member.plan}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-6 sm:gap-10">
                  <div>
                    <p className="text-xs text-zinc-500">30日</p>
                    <p className="text-xl font-bold tabular-nums text-red-400">
                      {prediction.probability30Days}%
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-zinc-500">60日</p>
                    <p className="text-xl font-bold tabular-nums text-orange-400/90">
                      {prediction.probability60Days}%
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-zinc-500">想定損失額（目安）</p>
      ...
      <section>
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">収益リスクAI</h2>
          <p className="mt-2 text-sm text-zinc-500">退会確率ベースの損失試算と、守るべき会員規模</p>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-red-500/30 bg-red-950/20 px-6 py-8">
            ...
            <p className="mt-3 text-4xl font-bold tabular-nums text-red-400">
          ...
          <div className="rounded-2xl border border-red-500/20 bg-zinc-900 px-6 py-8">
            <p className="text-sm text-zinc-400">年間危険売上</p>
            <p className="mt-3 text-4xl font-bold tabular-nums text-red-400">
              {formatYen(revenueRisk.annualDanger)}
            </p>
            <p className="mt-1 text-xs text-zinc-600">高リスク会員の年間売上合算</p>
          </div>
          <div className="rounded-2xl border border-zinc-700 bg-zinc-900 px-6 py-8">
            <p className="text-sm text-zinc-400">守るべき会員数</p>
            <p className="mt-3 text-4xl font-bold tabular-nums text-white">
              {revenueRisk.membersToDefend}
            </p>
            <p className="mt-1 text-xs text-zinc-600">中・高リスク（介入推奨層）</p>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">店舗別サマリー</h2>
          <p className="mt-2 text-sm text-zinc-500">損失見込みが大きい店舗から上位5店舗</p>
        </div>
          ...
          {topStores.map((s) => (
            ...
            >
              ...
              <div className="mt-5 grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-zinc-500">推定継続率</p>
                  <p className="mt-1 text-xl font-bold text-emerald-400">
                    {Math.round(s.estimatedRetentionRate)}%
                  </p>
                </div>
                <div>
                  <p className="text-zinc-500">高リスク</p>
                  <p className="mt-1 text-xl font-bold text-red-400">{s.highRiskMembers}名</p>
                </div>
                <div className="col-span-2">
                  <p className="text-zinc-500">来月損失予測</p>
      ...
      <section className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-950/30 to-zinc-950 px-6 py-10 sm:px-10">
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">本部向け改善提案AI</h2>
          <span
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              {hqPlan.priorityStores.length === 0 ? (
                <p className="mt-2 text-sm text-zinc-500">該当なし</p>
              ) : (
