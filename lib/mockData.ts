import { Member, Visit, Intervention, Task } from "@/types";

export const members: Member[] = [
  {
    id: "1",
    name: "田中太郎",
    plan: "Premium",
    storeName: "三軒茶屋本店",
    joinDate: "2023-06-15",
    lastVisitDate: "2024-01-15",
    visitInterval: "3 days",
    riskScore: 85,
    interventionStatus: "High Priority",
    recommendedIntervention: "Immediate follow-up consultation required",
    notes: "Member has been showing decreased engagement. Risk score has increased significantly over the past month.",
    monthlyRevenue: 39800,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "21:00-22:00",
    reservationDifficultyLevel: "difficult",
    preferredWeekday: "月",
    preferredHour: 19,
    assignedTrainer: "山本トレーナー",
    isPriceRevisionTarget: true,
    priceRevisionBeforeRevenue: 39800,
    priceRevisionAfterRevenue: 43000,
  },
  {
    id: "2",
    name: "佐藤花子",
    plan: "Standard",
    storeName: "人形町店",
    joinDate: "2023-08-20",
    lastVisitDate: "2024-01-20",
    visitInterval: "5 days",
    riskScore: 65,
    interventionStatus: "Monitor",
    recommendedIntervention: "Nutrition plan review",
    notes: "来店は安定しているが、最近は来店間隔がやや拡大傾向。次回セッションで生活習慣とモチベーションを確認する。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "火",
    preferredHour: 18,
    assignedTrainer: "佐々木トレーナー",
    isPriceRevisionTarget: true,
    priceRevisionBeforeRevenue: 26000,
    priceRevisionAfterRevenue: 28000,
  },
  {
    id: "3",
    name: "鈴木一郎",
    plan: "Basic",
    storeName: "水天宮前店",
    joinDate: "2023-11-10",
    lastVisitDate: "2024-01-22",
    visitInterval: "2 days",
    riskScore: 35,
    interventionStatus: "Normal",
    recommendedIntervention: "Continue current program",
    notes: "来店ペースが安定しており、プログラムを着実に継続中。現状維持で問題なし。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "水",
    preferredHour: 20,
    assignedTrainer: "山本トレーナー",
    isPriceRevisionTarget: false,
  },
  {
    id: "4",
    name: "山田次郎",
    plan: "Premium",
    storeName: "渋谷店",
    joinDate: "2023-05-05",
    lastVisitDate: "2024-01-10",
    visitInterval: "10 days",
    riskScore: 92,
    interventionStatus: "Urgent",
    recommendedIntervention: "Urgent intervention - schedule emergency consultation",
    notes: "Critical situation. Member has not visited in 10 days. Risk score is extremely high.",
    monthlyRevenue: 39800,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: undefined,
    reservationDifficultyLevel: "difficult",
    preferredWeekday: "木",
    preferredHour: 19,
    hasCancellationHistory: true,
    assignedTrainer: "佐々木トレーナー",
    isPriceRevisionTarget: true,
    priceRevisionBeforeRevenue: 39800,
    priceRevisionAfterRevenue: 43000,
  },
  {
    id: "5",
    name: "伊藤美咲",
    plan: "Standard",
    storeName: "中野店",
    joinDate: "2023-09-12",
    lastVisitDate: "2024-01-21",
    visitInterval: "4 days",
    riskScore: 45,
    interventionStatus: "Normal",
    recommendedIntervention: "Workout plan adjustment",
    notes: "Good progress overall. Minor adjustments needed to optimize results.",
    monthlyRevenue: 26000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "medium",
    preferredWeekday: "金",
    preferredHour: 18,
    assignedTrainer: "山本トレーナー",
    isPriceRevisionTarget: true,
    priceRevisionBeforeRevenue: 26000,
    priceRevisionAfterRevenue: 28000,
  },
  {
    id: "6",
    name: "高橋健太",
    plan: "Premium",
    storeName: "三軒茶屋本店",
    joinDate: "2023-07-01",
    lastVisitDate: "2024-01-23",
    visitInterval: "1 day",
    riskScore: 25,
    interventionStatus: "Normal",
    recommendedIntervention: "Continue excellent progress",
    notes: "Highly motivated member with consistent attendance. Risk score is very low.",
    monthlyRevenue: 33000,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "土",
    preferredHour: 20,
    assignedTrainer: "佐々木トレーナー",
    isPriceRevisionTarget: false,
  },
  {
    id: "13",
    name: "渡辺さくら",
    plan: "デュアル月8",
    currentPlan: "デュアル月8",
    storeName: "人形町店",
    joinDate: "2023-12-01",
    lastVisitDate: "2024-01-20",
    visitInterval: "6 days",
    riskScore: 55,
    interventionStatus: "Monitor",
    recommendedIntervention: "Plan transition consideration",
    notes: "デュアルプランで継続中。移行先の検討が必要。",
    monthlyRevenue: 33000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "火",
    preferredHour: 18,
  },
  // 人形町店：デモ会員追加（店舗切替・セッション入力用）
  {
    id: "ningyocho_demo_1",
    name: "山口拓海",
    plan: "Standard",
    storeName: "人形町店",
    joinDate: "2023-09-01",
    lastVisitDate: "2024-01-21",
    visitInterval: "4 days",
    riskScore: 40,
    interventionStatus: "Normal",
    notes: "来店ペースは安定。トレーニングへの意欲は高く、次回は新しいメニューへの移行タイミングを検討。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "月",
    preferredHour: 19,
    assignedTrainer: "我妻",
  },
  {
    id: "ningyocho_demo_2",
    name: "石田美月",
    plan: "Basic",
    storeName: "人形町店",
    joinDate: "2023-10-12",
    lastVisitDate: "2024-01-22",
    visitInterval: "5 days",
    riskScore: 35,
    interventionStatus: "Normal",
    notes: "入会4ヶ月。体重管理を主目的としており順調。次回は食事改善の具体策を提案予定。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "火",
    preferredHour: 18,
    assignedTrainer: "竹内",
  },
  {
    id: "ningyocho_demo_3",
    name: "森田健介",
    plan: "Premium",
    storeName: "人形町店",
    joinDate: "2023-08-05",
    lastVisitDate: "2024-01-20",
    visitInterval: "11 days",
    riskScore: 82,
    interventionStatus: "High Priority",
    hasCancellationHistory: true,
    notes: "直近3週間で来店が途絶えがち。前回セッション時にモチベーション低下の発言あり。早急にフォローコールが必要。",
    monthlyRevenue: 39800,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "medium",
    preferredWeekday: "水",
    preferredHour: 20,
    assignedTrainer: "小野",
  },
  {
    id: "ningyocho_demo_4",
    name: "小林里奈",
    plan: "Standard",
    storeName: "人形町店",
    joinDate: "2023-11-18",
    lastVisitDate: "2024-01-19",
    visitInterval: "6 days",
    riskScore: 68,
    interventionStatus: "Monitor",
    notes: "直近2回をキャンセル。次回予約が未確定のため、優先的に予約確保のサポートが必要。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: undefined,
    reservationDifficultyLevel: "medium",
    preferredWeekday: "木",
    preferredHour: 19,
    assignedTrainer: "我妻",
  },
  {
    id: "ningyocho_demo_5",
    name: "松本悠斗",
    plan: "Basic",
    storeName: "人形町店",
    joinDate: "2023-12-20",
    lastVisitDate: "2024-01-23",
    visitInterval: "4 days",
    riskScore: 30,
    interventionStatus: "Normal",
    notes: "入会1ヶ月の新規会員。来店意欲は高い。プログラム定着を確認しつつ、習慣化の支援を継続。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "金",
    preferredHour: 18,
    assignedTrainer: "竹内",
  },
  {
    id: "14",
    name: "中島健",
    plan: "デュアル月8",
    currentPlan: "デュアル月8",
    storeName: "渋谷店",
    joinDate: "2023-10-15",
    lastVisitDate: "2024-01-22",
    visitInterval: "2 days",
    riskScore: 30,
    interventionStatus: "Normal",
    recommendedIntervention: "Plan transition consideration",
    notes: "デュアルプランで順調に継続。移行先の提案が可能。",
    monthlyRevenue: 33000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "水",
    preferredHour: 19,
  },
  {
    id: "15",
    name: "前田由美",
    plan: "デュアル月8",
    currentPlan: "デュアル月8",
    storeName: "中野店",
    joinDate: "2023-11-20",
    lastVisitDate: "2024-01-15",
    visitInterval: "8 days",
    riskScore: 60,
    interventionStatus: "Monitor",
    recommendedIntervention: "Plan transition consideration",
    notes: "デュアルプランで来店間隔が空きがち。移行先の検討が必要。",
    monthlyRevenue: 33000,
    preferredTimeSlot: "17:00-18:00",
    bookedTimeSlot: "17:00-18:00",
    reservationDifficultyLevel: "medium",
    preferredWeekday: "木",
    preferredHour: 17,
    hasCancellationHistory: true,
  },
  {
    id: "16",
    name: "藤原大輔",
    plan: "デュアル月8",
    currentPlan: "デュアル月8",
    storeName: "三軒茶屋本店",
    joinDate: "2024-01-05",
    lastVisitDate: "2024-01-24",
    visitInterval: "3 days",
    riskScore: 40,
    interventionStatus: "Normal",
    recommendedIntervention: "Plan transition consideration",
    notes: "新規入会。デュアルプランで順調に継続中。",
    monthlyRevenue: 33000,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "金",
    preferredHour: 20,
  },
  {
    id: "17",
    name: "村上麻衣",
    plan: "デュアル月8",
    currentPlan: "デュアル月8",
    storeName: "水天宮前店",
    joinDate: "2023-09-10",
    lastVisitDate: "2024-01-18",
    visitInterval: "12 days",
    riskScore: 70,
    interventionStatus: "High Priority",
    recommendedIntervention: "Plan transition consideration",
    notes: "デュアルプランで来店間隔が長くなっている。移行先の検討が急務。",
    monthlyRevenue: 33000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "medium",
    preferredWeekday: "土",
    preferredHour: 19,
  },
  // ──────────────────────────────
  // 水天宮前店：デモ会員（店舗切替デモ用）
  // ──────────────────────────────
  {
    id: "suitengumae_demo_1",
    name: "山田 花音",
    plan: "Premium",
    storeName: "水天宮前店",
    joinDate: "2023-10-05",
    lastVisitDate: "2024-01-24",
    visitInterval: "3 days",
    riskScore: 35,
    interventionStatus: "Normal",
    recommendedIntervention: "Continue current program",
    notes: "最近は来店が安定。現状維持でOK。",
    monthlyRevenue: 39800,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "火",
    preferredHour: 19,
    assignedTrainer: "小野",
  },
  {
    id: "suitengumae_demo_2",
    name: "佐藤 恒一",
    plan: "Standard",
    storeName: "水天宮前店",
    joinDate: "2023-07-18",
    lastVisitDate: "2024-01-19",
    visitInterval: "8 days",
    riskScore: 58,
    interventionStatus: "Monitor",
    recommendedIntervention: "Follow-up consultation recommended",
    notes: "少し来店間隔が空きがち。経過観察しつつフォロー。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "medium",
    preferredWeekday: "水",
    preferredHour: 20,
    assignedTrainer: "渡部",
  },
  {
    id: "suitengumae_demo_3",
    name: "高橋 美咲",
    plan: "Basic",
    storeName: "水天宮前店",
    joinDate: "2023-12-12",
    lastVisitDate: "2024-01-23",
    visitInterval: "5 days",
    riskScore: 42,
    interventionStatus: "Normal",
    recommendedIntervention: "Continue current program",
    notes: "最近は順調。フォーム改善の継続を提案。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "木",
    preferredHour: 18,
    assignedTrainer: "藤本",
  },
  {
    id: "suitengumae_demo_4",
    name: "伊藤 拓真",
    plan: "Premium",
    storeName: "水天宮前店",
    joinDate: "2023-04-22",
    lastVisitDate: "2024-01-12",
    visitInterval: "12 days",
    riskScore: 86,
    interventionStatus: "High Priority",
    recommendedIntervention: "Immediate follow-up consultation required",
    notes: "来店間隔が空いているため高優先でフォロー。",
    monthlyRevenue: 39800,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: undefined,
    reservationDifficultyLevel: "difficult",
    preferredWeekday: "金",
    preferredHour: 19,
    hasCancellationHistory: true,
    assignedTrainer: "小野",
    isPriceRevisionTarget: true,
    priceRevisionBeforeRevenue: 39800,
    priceRevisionAfterRevenue: 43000,
  },
  {
    id: "suitengumae_demo_5",
    name: "中村 彩乃",
    plan: "Standard",
    storeName: "水天宮前店",
    joinDate: "2023-09-28",
    lastVisitDate: "2024-01-22",
    visitInterval: "4 days",
    riskScore: 38,
    interventionStatus: "Normal",
    recommendedIntervention: "Continue current program",
    notes: "最近は安定。次回は目標設定の再確認。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "月",
    preferredHour: 18,
    assignedTrainer: "渡部",
  },
  // 水天宮前店：追加デモ会員（既存 suitengumae_demo_* と名前重複なし）
  {
    id: "suitengumae_demo_b_1",
    name: "清水あかり",
    plan: "Standard",
    storeName: "水天宮前店",
    joinDate: "2023-08-08",
    lastVisitDate: "2024-01-21",
    visitInterval: "5 days",
    riskScore: 36,
    interventionStatus: "Normal",
    notes: "来店ペース安定。筋力アップを目標にコンスタントに通えている。次回は目標の進捗確認を予定。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: "19:00-20:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "火",
    preferredHour: 19,
    assignedTrainer: "小野",
  },
  {
    id: "suitengumae_demo_b_2",
    name: "林大輔",
    plan: "Premium",
    storeName: "水天宮前店",
    joinDate: "2023-07-25",
    lastVisitDate: "2024-01-20",
    visitInterval: "4 days",
    riskScore: 44,
    interventionStatus: "Normal",
    notes: "体重管理と体力維持が目的。週1〜2回ペースで継続中。次回は3ヶ月の成果振り返りを実施予定。",
    monthlyRevenue: 39800,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "水",
    preferredHour: 18,
    assignedTrainer: "渡部",
  },
  {
    id: "suitengumae_demo_b_3",
    name: "岡田さくら",
    plan: "Basic",
    storeName: "水天宮前店",
    joinDate: "2023-11-02",
    lastVisitDate: "2024-01-22",
    visitInterval: "6 days",
    riskScore: 33,
    interventionStatus: "Normal",
    notes: "入会半年。フォームの改善が顕著で本人の満足度も高い。継続意欲は安定している。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "20:00-21:00",
    bookedTimeSlot: "20:00-21:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "木",
    preferredHour: 20,
    assignedTrainer: "藤本",
  },
  {
    id: "suitengumae_demo_b_4",
    name: "長谷川翼",
    plan: "Standard",
    storeName: "水天宮前店",
    joinDate: "2023-09-14",
    lastVisitDate: "2024-01-18",
    visitInterval: "7 days",
    riskScore: 62,
    interventionStatus: "Monitor",
    notes: "来店はやや不定期。次回予約が入っていないため、早めに予約確保の声がけが必要。",
    monthlyRevenue: 26000,
    preferredTimeSlot: "19:00-20:00",
    bookedTimeSlot: undefined,
    reservationDifficultyLevel: "medium",
    preferredWeekday: "金",
    preferredHour: 19,
    assignedTrainer: "小野",
  },
  {
    id: "suitengumae_demo_b_5",
    name: "井上結衣",
    plan: "Basic",
    storeName: "水天宮前店",
    joinDate: "2023-12-08",
    lastVisitDate: "2024-01-23",
    visitInterval: "3 days",
    riskScore: 28,
    interventionStatus: "Normal",
    notes: "入会5ヶ月。体型改善の実感が出てきており、モチベーションが高い。継続支援を継続。",
    monthlyRevenue: 16000,
    preferredTimeSlot: "18:00-19:00",
    bookedTimeSlot: "18:00-19:00",
    reservationDifficultyLevel: "easy",
    preferredWeekday: "土",
    preferredHour: 18,
    assignedTrainer: "渡部",
  },
];

// 訪問履歴データ
export const visits: Visit[] = [
  {
    id: "v1",
    memberId: "1",
    visitDate: "2024-01-15",
  },
  {
    id: "v2",
    memberId: "1",
    visitDate: "2024-01-12",
  },
  {
    id: "v3",
    memberId: "1",
    visitDate: "2024-01-08",
  },
  {
    id: "v4",
    memberId: "2",
    visitDate: "2024-01-20",
  },
  {
    id: "v5",
    memberId: "2",
    visitDate: "2024-01-15",
  },
  {
    id: "v6",
    memberId: "2",
    visitDate: "2024-01-10",
  },
  {
    id: "v7",
    memberId: "3",
    visitDate: "2024-01-22",
  },
  {
    id: "v8",
    memberId: "3",
    visitDate: "2024-01-20",
  },
  {
    id: "v9",
    memberId: "3",
    visitDate: "2024-01-18",
  },
  {
    id: "v10",
    memberId: "4",
    visitDate: "2024-01-10",
  },
  {
    id: "v11",
    memberId: "4",
    visitDate: "2024-01-05",
  },
  {
    id: "v12",
    memberId: "4",
    visitDate: "2023-12-28",
  },
  {
    id: "v13",
    memberId: "5",
    visitDate: "2024-01-21",
  },
  {
    id: "v14",
    memberId: "5",
    visitDate: "2024-01-17",
  },
  {
    id: "v15",
    memberId: "5",
    visitDate: "2024-01-14",
  },
  {
    id: "v16",
    memberId: "6",
    visitDate: "2024-01-23",
  },
  {
    id: "v17",
    memberId: "6",
    visitDate: "2024-01-22",
  },
];

// 介入履歴データ
export const interventions: Intervention[] = [
  {
    id: "i1",
    memberId: "1",
    type: "Follow-up Consultation",
    action: "Conducted phone consultation to discuss progress",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2024-01-15",
  },
  {
    id: "i2",
    memberId: "1",
    type: "Nutrition Review",
    action: "Reviewed and updated nutrition plan",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2024-01-08",
  },
  {
    id: "i3",
    memberId: "1",
    type: "Program Adjustment",
    action: "Modified workout program based on feedback",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2023-12-20",
  },
  {
    id: "i4",
    memberId: "2",
    type: "Progress Review",
    action: "Monthly progress check and goal setting",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2024-01-20",
  },
  {
    id: "i5",
    memberId: "2",
    type: "Nutrition Plan Update",
    action: "Updated meal plan for better results",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2024-01-05",
  },
  {
    id: "i6",
    memberId: "3",
    type: "Monthly Check-in",
    action: "Regular monthly consultation",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2024-01-18",
  },
  {
    id: "i7",
    memberId: "4",
    type: "Emergency Consultation",
    action: "Urgent consultation due to high risk score",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2024-01-10",
  },
  {
    id: "i8",
    memberId: "4",
    type: "Risk Assessment",
    action: "Comprehensive risk assessment and intervention plan",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2023-12-28",
  },
  {
    id: "i9",
    memberId: "4",
    type: "Program Review",
    action: "Full program review and adjustment",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2023-12-15",
  },
  {
    id: "i10",
    memberId: "5",
    type: "Plan Adjustment",
    action: "Adjusted workout plan for better engagement",
    status: "Completed",
    trainer: "山本トレーナー",
    createdAt: "2024-01-14",
  },
  {
    id: "i11",
    memberId: "5",
    type: "Progress Review",
    action: "Quarterly progress review session",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2024-01-01",
  },
  {
    id: "i12",
    memberId: "6",
    type: "Initial Assessment",
    action: "Initial consultation and goal setting",
    status: "Completed",
    trainer: "佐々木トレーナー",
    createdAt: "2024-01-22",
  },
];

// タスクデータ
export const tasks: Task[] = [
  {
    id: "t1",
    memberId: "1",
    memberName: "田中太郎",
    action: "Follow-up consultation",
    status: "pending",
    assignedTrainer: "山本トレーナー",
    dueDate: "2024-01-25",
  },
  {
    id: "t2",
    memberId: "2",
    memberName: "佐藤花子",
    action: "Nutrition plan review",
    status: "in progress",
    assignedTrainer: "佐々木トレーナー",
    dueDate: "2024-01-24",
  },
  {
    id: "t3",
    memberId: "3",
    memberName: "鈴木一郎",
    action: "Progress check",
    status: "done",
    assignedTrainer: "山本トレーナー",
    dueDate: "2024-01-23",
  },
  {
    id: "t4",
    memberId: "4",
    memberName: "山田次郎",
    action: "Emergency intervention",
    status: "pending",
    assignedTrainer: "佐々木トレーナー",
    dueDate: "2024-01-26",
  },
  {
    id: "t5",
    memberId: "5",
    memberName: "伊藤美咲",
    action: "Workout plan adjustment",
    status: "in progress",
    assignedTrainer: "山本トレーナー",
    dueDate: "2024-01-25",
  },
  {
    id: "t6",
    memberId: "6",
    memberName: "高橋健太",
    action: "Initial assessment",
    status: "done",
    assignedTrainer: "佐々木トレーナー",
    dueDate: "2024-01-22",
  },
  {
    id: "t7",
    memberId: "1",
    memberName: "田中太郎",
    action: "Risk assessment review",
    status: "pending",
    assignedTrainer: "山本トレーナー",
    dueDate: "2024-01-27",
  },
  // 人形町店 追加タスク
  {
    id: "t_n1",
    memberId: "ningyocho_demo_3",
    memberName: "森田健介",
    action: "退会リスク高：来店11日空き・フォローコール要",
    status: "pending",
    assignedTrainer: "小野",
    dueDate: "2026-05-19",
    priority: "high",
  },
  {
    id: "t_n2",
    memberId: "ningyocho_demo_4",
    memberName: "小林里奈",
    action: "未予約会員：次回予約の確保サポート",
    status: "in progress",
    assignedTrainer: "我妻",
    dueDate: "2026-05-20",
    priority: "medium",
  },
  {
    id: "t_n3",
    memberId: "2",
    memberName: "佐藤花子",
    action: "来店間隔増加：生活習慣・モチベヒアリング",
    status: "pending",
    assignedTrainer: "佐々木トレーナー",
    dueDate: "2026-05-22",
    priority: "medium",
  },
  {
    id: "t_n4",
    memberId: "13",
    memberName: "渡辺さくら",
    action: "デュアルプラン移行先の提案検討",
    status: "pending",
    assignedTrainer: "佐々木トレーナー",
    dueDate: "2026-05-26",
    priority: "low",
  },
  // 水天宮前店 追加タスク
  {
    id: "t_s1",
    memberId: "suitengumae_demo_4",
    memberName: "伊藤 拓真",
    action: "緊急介入：来店12日空き・キャンセル歴あり",
    status: "pending",
    assignedTrainer: "小野",
    dueDate: "2026-05-18",
    priority: "high",
  },
  {
    id: "t_s2",
    memberId: "17",
    memberName: "村上麻衣",
    action: "来店間隔12日：目標再設定・プラン見直し提案",
    status: "in progress",
    assignedTrainer: "渡部",
    dueDate: "2026-05-21",
    priority: "medium",
  },
  {
    id: "t_s3",
    memberId: "suitengumae_demo_b_4",
    memberName: "長谷川翼",
    action: "予約枠未確定：来週分の予約確保を優先対応",
    status: "pending",
    assignedTrainer: "小野",
    dueDate: "2026-05-22",
    priority: "medium",
  },
  {
    id: "t_s4",
    memberId: "suitengumae_demo_2",
    memberName: "佐藤 恒一",
    action: "来店頻度低下：週次フォローの開始",
    status: "pending",
    assignedTrainer: "渡部",
    dueDate: "2026-05-23",
    priority: "low",
  },
];

// Helper functions to get data by member ID
// 注意: これらの関数は後方互換性のために残していますが、
// 新しいコードでは lib/repositories 経由でアクセスすることを推奨します
export function getMemberById(id: string): Member | undefined {
  return members.find((member) => member.id === id);
}

export function getVisitsByMemberId(memberId: string): Visit[] {
  return visits.filter((visit) => visit.memberId === memberId);
}

export function getInterventionsByMemberId(memberId: string): Intervention[] {
  return interventions.filter((intervention) => intervention.memberId === memberId);
}

export function getTasksByMemberId(memberId: string): Task[] {
  return tasks.filter((task) => task.memberId === memberId);
}
